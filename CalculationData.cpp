	...
	if ( textFile.Open( filePath ) )
	{
		...
		for ( int k = 1; k < 21; k++ )
		{
			for ( int l = 0; l < 8; l++ )
			{
				...
				if ( std::abs( backPoint.RayDifference - UnassignedDoubleValue ) > Epsilon && std::abs( point.RayDifference - UnassignedDoubleValue ) > Epsilon && std::abs( forwardPoint.RayDifference - UnassignedDoubleValue ) > Epsilon && std::abs( backPoint.Height - UnassignedIntValue ) > Epsilon && std::abs( point.Height - UnassignedIntValue ) > Epsilon && std::abs( forwardPoint.Height - UnassignedIntValue ) > Epsilon )
				{
